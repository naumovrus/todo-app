...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	tx, err := r.db.Begin() // Для транзакций используется метод Begin()
	if err != nil {
		return 0, err
	}
	var id int
	...
	if err := row.Scan(&id); err != nil {
		tx.Rollback() // В случае, когда у нас ошибка в одном действии из транзакций, откатываем
		return 0, err
	}
...
func (r *TodoListPostgres) Delete(userId, listId int) error {
	query := fmt.Sprintf(`DELETE FROM %s tl USING %s ul WHERE tl.id = ul.list_id 
						AND ul.user_id = $1 AND ul.list_id = $2`, todoListsTable, userListsTable)
